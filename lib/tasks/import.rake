...
namespace :import do
  ...
  task :json_file, [:json_content] => :environment do |_, args|
    activity_list = if args.key?(:json_content) && !args[:json_content].nil?
                      args[:json_content]
                    else
                      JSON.parse(File.read(Rails.root.join('db', 'seeds', 'json', 'madrid.json')))
                    end

    activity_list.each do |activity|
      latlng = activity['latlng']
      activity_object = Activity.create(activity.except('opening_hours', 'latlng')
          .merge('latitude' => latlng[0], 'longitude' => latlng[1]))
